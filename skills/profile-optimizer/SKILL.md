...
> output, refresh, content engine), with `skills/linkedin-writer/SKILL.md` for the
> writing itself (SEO rules, tone, humanizer pass).
> LinkedIn is never fetched or scraped: ask for screenshots. The Naukri and LinkedIn notes below remain as a quick reference only.
